...
return r;},parseAjaxResponse:function(x,r,e){var parsed={},re=jQuery('#'+r).empty(),err='',successmsg='';if(x&&typeof x==='object'&&x.getElementsByTagName('wp_ajax')){parsed.responses=[];parsed.errors=false;jQuery('response',x).each(function(){var th=jQuery(this),child=jQuery(this.firstChild),response;response={action:th.attr('action'),what:child.get(0).nodeName,id:child.attr('id'),oldId:child.attr('old_id'),position:child.attr('position')};response.data=jQuery('response_data',child).text();if(jQuery('body').hasClass('edit-tags-php')){successmsg+=response.data;}
response.supplemental={};if(!jQuery('supplemental',child).children().each(function(){response.supplemental[this.nodeName]=jQuery(this).text();}).length){response.supplemental=false;}
...
parsed.responses.push(response);});if(err.length){re.html('<div class="error">'+err+'</div>');wp.a11y.speak(err);}else if(successmsg.length){re.html('<div class="updated notice is-dismissible"><p>'+successmsg+'</p></div>');jQuery(document).trigger('wp-updates-notice-added');wp.a11y.speak(successmsg);}
return parsed;}
